...
import { SketchBullDetective, SketchTarget, SketchNewspaper } from '@/components/ui/Sketches';

function CardSketch({ type }: { type: string }) {
  if (type === 'roast') return <SketchBullDetective className="text-muted-light opacity-40" size={44} />;
  if (type === 'pick') return <SketchTarget className="text-muted-light opacity-40" size={44} />;
  return <SketchNewspaper className="text-muted-light opacity-40" size={44} />;
}
      ...
      <article className="border border-card-border bg-background rounded-xl overflow-hidden hover:shadow-lg hover:border-accent/30 transition-all active:scale-[0.99]">
        ...
        <div className="p-4 sm:p-6">
          <div className="flex items-start gap-3">
            {/* Sketch illustration — desktop only */}
            <div className="hidden sm:flex flex-shrink-0 mt-1">
              <CardSketch type={article.type} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                <span className="text-base sm:hidden" aria-hidden="true">{icon}</span>
